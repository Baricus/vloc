...
Declare Scope iris_expr_scope.
Delimit Scope iris_expr_scope with Ei.
...
properly. *)
Notation "# l" := (LitV l%Z%V%stdpp) (at level 20, l at level 20, format "# l").

(** Syntax inspired by Coq/Ocaml. Constructions with higher precedence come
    first. *)
Notation "( e1 , e2 , .. , en )" := (Pair .. (Pair e1 e2) .. en) : iris_expr_scope.
...
Notation "'match:' e0 'with' 'InjL' x1 => e1 | 'InjR' x2 => e2 'end'" :=
  ...
  (e0, x1, e1, x2, e2 at level 200,
   format "'[hv' 'match:'  e0  'with'  '/  ' '[' 'InjL'  x1  =>  '/  ' e1 ']'  '/' '[' |  'InjR'  x2  =>  '/  ' e2 ']'  '/' 'end' ']'") : iris_expr_scope.
Notation "'match:' e0 'with' 'InjR' x1 => e1 | 'InjL' x2 => e2 'end'" :=
  (Match e0 x2%binder e2 x1%binder e1)
  (e0, x1, e1, x2, e2 at level 200, only parsing) : iris_expr_scope.

Notation "()" := LitUnit : val_scope.
Notation "! e" := (Load e%E) (at level 20, e at level 20, right associativity) : iris_expr_scope.
Notation "'ref' e" := (Alloc e%E) (at level 10) : iris_expr_scope.
Notation "- e" := (UnOp MinusUnOp e%E) : iris_expr_scope.

Notation "e1 + e2" := (BinOp PlusOp e1%E e2%E) : iris_expr_scope.
Notation "e1 +ₗ e2" := (BinOp OffsetOp e1%E e2%E) : iris_expr_scope.
Notation "e1 - e2" := (BinOp MinusOp e1%E e2%E) : iris_expr_scope.
Notation "e1 * e2" := (BinOp MultOp e1%E e2%E) : iris_expr_scope.
Notation "e1 `quot` e2" := (BinOp QuotOp e1%E e2%E) : iris_expr_scope.
Notation "e1 `rem` e2" := (BinOp RemOp e1%E e2%E) : iris_expr_scope.
Notation "e1 ≪ e2" := (BinOp ShiftLOp e1%E e2%E) : iris_expr_scope.
Notation "e1 ≫ e2" := (BinOp ShiftROp e1%E e2%E) : iris_expr_scope.

Notation "e1 ≤ e2" := (BinOp LeOp e1%E e2%E) : iris_expr_scope.
Notation "e1 < e2" := (BinOp LtOp e1%E e2%E) : iris_expr_scope.
Notation "e1 = e2" := (BinOp EqOp e1%E e2%E) : iris_expr_scope.
Notation "e1 ≠ e2" := (UnOp NegOp (BinOp EqOp e1%E e2%E)) : iris_expr_scope.

Notation "~ e" := (UnOp NegOp e%E) (at level 75, right associativity) : iris_expr_scope.
(* The unicode ← is already part of the notation "_ ← _; _" for bind. *)
Notation "e1 <- e2" := (Store e1%E e2%E) (at level 80) : iris_expr_scope.
...
Notation "'rec:' f x := e" := (Rec f%binder x%binder e%E)
  (at level 200, f at level 1, x at level 1, e at level 200,
   format "'[' 'rec:'  f  x  :=  '/  ' e ']'") : iris_expr_scope.
Notation "'rec:' f x := e" := (RecV f%binder x%binder e%E)
  (at level 200, f at level 1, x at level 1, e at level 200,
   format "'[' 'rec:'  f  x  :=  '/  ' e ']'") : val_scope.
Notation "'if:' e1 'then' e2 'else' e3" := (If e1%E e2%E e3%E)
  (at level 200, e1, e2, e3 at level 200) : iris_expr_scope.
...
Notation "'rec:' f x y .. z := e" := (Rec f%binder x%binder (Lam y%binder .. (Lam z%binder e%E) ..))
  (at level 200, f, x, y, z at level 1, e at level 200,
   format "'[' 'rec:'  f  x  y  ..  z  :=  '/  ' e ']'") : iris_expr_scope.
...
Notation "'let:' x := e1 'in' e2" := (Lam x%binder e2%E e1%E)
  (at level 200, x at level 1, e1, e2 at level 200,
   format "'[' 'let:'  x  :=  '[' e1 ']'  'in'  '/' e2 ']'") : iris_expr_scope.
Notation "e1 ;; e2" := (Lam BAnon e2%E e1%E)
  (at level 100, e2 at level 200,
   format "'[' '[hv' '[' e1 ']' ;;  ']' '/' e2 ']'") : iris_expr_scope.

(* Shortcircuit Boolean connectives *)
Notation "e1 && e2" :=
  (If e1%E e2%E (LitV (LitBool false))) (only parsing) : iris_expr_scope.
Notation "e1 || e2" :=
  (If e1%E (LitV (LitBool true)) e2%E) (only parsing) : iris_expr_scope.
...
Notation "'match:' e0 'with' 'NONE' => e1 | 'SOME' x => e2 'end'" :=
  ...
  (e0, e1, x, e2 at level 200, only parsing) : iris_expr_scope.
Notation "'match:' e0 'with' 'SOME' x => e2 | 'NONE' => e1 'end'" :=
  (Match e0 BAnon e1 x%binder e2)
  (e0, e1, x, e2 at level 200, only parsing) : iris_expr_scope.

Notation ResolveProph e1 e2 := (Resolve Skip e1 e2) (only parsing).
Notation "'resolve_proph:' p 'to:' v" := (ResolveProph p v) (at level 100) : iris_expr_scope.
